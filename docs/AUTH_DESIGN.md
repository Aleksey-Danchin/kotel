...
```
messenger://auth/callback
```

Когда сервер редиректит на `messenger://auth/callback?code=...` — ОС перехватывает URL и открывает приложение. В Expo это реализуется через `expo-linking`.
...
redirect_uri начинается с messenger:// → clientType = "expo"
```
...
- Deep Links (`messenger://auth/callback`) для получения callback от сервера
